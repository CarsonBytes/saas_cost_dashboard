...
flowchart TD
    ...
    subgraph Hub["This app — D:\llm-usage-dashboard"]
        ...
        SV["services.py<br/>10 agents, reachability probes"]
        UI["app.py — NiceGUI"]
    end

    TG[("Telegram")]
    OTHER["quant / events / study / portfolio /<br/>regtech radar / LinkedIn engine / 3 more AI projects"]
...
### 2026-09-05 — Quant Paper's frequent lock-ups root-caused and fixed; daily prune + WSL2 memory reclaim
- **A per-agent liveness-flapping guard, closing a gap the 2026-08-15 dependency-flapping fix never covered.** Quant Trading (Paper) had 24 auto-heal restarts logged against 2-3 for every other agent — traced live, not guessed: its own IB Gateway sidecar periodically self-restarts on a routine IBKR 2FA-session timeout (`on2fatimeout: restart`, especially frequent on weekends — IBKR's own weekly server reset). That's already correctly handled on the quant side by its own `gateway-login-watchdog.sh` (escalating retries, phone-push alerts) — no changes needed there. But each sidecar restart briefly breaks the dashboard's own IB connection, making it miss one liveness probe, which used to restart the *dashboard* immediately — pointless, since the dashboard process isn't broken, and the restart interrupts an already-in-progress reconnect instead of helping it, repeating until 3 strikes locked it. `noc.py`'s `_restart_warranted()` now requires 2 *consecutive* missed liveness cycles before authorizing a restart, mirroring the existing shared-dependency streak guard exactly. New embedded test (`test_liveness_flapping_replay`) replays the real down/up/down/up pattern found live and asserts none of it should fire.
- **Daily Docker image/build-cache prune** (`D:\claude\docker-prune.ps1`, a silent Windows Scheduled Task at 4am — outside this repo, machine-wide) — `docker image prune -af --filter until=72h` + `docker builder prune -af`, keyed to a real finding: 27.8GB of images (94% reclaimable) and 27.88GB of build cache (22.38GB reclaimable) from repeated `docker compose up -d --build` cycles across every project on this host, never pruned. `until=72h` keeps anything touched in the last 3 days so an active rebuild still hits cache.
- **WSL2 memory reclaim enabled** (`C:\Users\Cap\.wslconfig`, `autoMemoryReclaim=gradual`) — `vmmem` was the single largest process on the host (7.7GB, no configured ceiling), and Windows had only ~2GB free at the time. Applying it needs a full `wsl --shutdown`; all 13 containers came back automatically within ~20s (`restart: unless-stopped` + `docker`/`cloudflared` both enabled via systemd — confirmed live, no manual `docker compose up` needed). Before/after: vmmem 7.70GB→5.49GB, Windows free memory 2.28GB→4.34GB. Part of a broader 5-point Docker/WSL2 memory-control spec (per-container caps and a hard WSL2 memory ceiling are sized-later follow-ups, deliberately deferred pending a few more days of the memory-tracking data below).

### 2026-09-04 — Per-agent memory tracking (Docker/WSL2 memory-control spec, item 1 of 5)
- **Every monitored agent card now shows a real memory reading** ("Memory: 342 MB", or "98 MB / 512 MB cap" for the two capped LinkedIn containers) next to its 7-day uptime — sourced from a new read-only `GET /stats` on `restart_proxy.py` (the Docker Engine API's per-container stats endpoint, real usage minus page-cache to match what `docker stats` itself shows; one call covers every allow-listed container) that `noc.py` fetches once per health cycle. No change to the dashboard's own zero-Docker-socket-access security boundary — restart-proxy already held the socket for restart/pause, this just adds one more read-only action to the same narrow allow-list. Visibility only for now — no alert threshold yet, since most containers have no configured cap to compare against.

### 2026-08-29 — LinkedIn Content Engine card; tab-revert and event-loop-blocking fixes; a real governance render-path bug found via the fix
- **New agent card for the LinkedIn content engine** (`linkedin.carsonng.com`, Cloudflare Access-gated, quarantine covers its dashboard + scheduler containers together). Liveness-only by design — no `project_tag`/`freshness_table` — since it doesn't write to Supabase or the shared LLM ledger at all yet.
- **Tabs no longer revert to Overview on refresh, and tab-clicking got faster.** Root cause of both: `dashboard_body` was a `@ui.refreshable` that tore down and rebuilt the whole tab strip (including `ui.tabs()`/`ui.tab_panels()`) on every data refresh, combined with an unreliable raw `.on("update:model-value", ...)` binding for tracking the active tab. Fixed by making `dashboard_body` a plain function built once per page load, splitting each tab's content into its own independently-refreshable function, and switching to NiceGUI's own `tabs.on_value_change()`. Six call sites that blocked the shared event loop on `fetch_stats()` (violating that function's own documented contract that callers must run it in a thread) were converted to `asyncio.to_thread`.
- **Found via the pre-push test gate, not guessed: `governance_view()`'s render path was calling `governance.get_audit_log()` — a direct, blocking Supabase fetch — instead of the cached `governance.cached_audit()`.** Added ~2s to every single page render and intermittently failed the render-time test suite. A first (wrong) diagnosis blamed `governance/engine.py`'s cache-refresh function holding a lock across its own network calls — a real anti-pattern, fixed anyway, but timing instrumentation proved it wasn't the actual cause of this particular regression.
- **Adding this 5th monitored service (2 links, one behind an Access redirect chain) exposed two more real, unrelated bugs.** `noc.py::_liveness()` probed a service's own links sequentially, so a 2-gated-link service paid both redirect chains one after another; now concurrent. Separately, the test suite let the app's background loops (`noc.refresh_health`, `governance.check_pending_rules`) hit real production endpoints on every iteration during a run, intermittently starving the same thread pool `fetch_stats` draws from and tipping unrelated tests past the render-readiness budget — `conftest.py` now seeds both caches for real exactly once per test session, then no-ops them for the rest of the run (test suite went from ~5-8 min and flaky to a consistent ~60-125s clean).

### 2026-08-26 (Phase 5) — solo-operator usability: auto-unlock with escalation, Telegram control plane, maintenance window, per-agent incidents
The lock's manual-only unlock protected against a multi-operator failure mode while taxing the only person who could clear it — a flapping dependency at 2am left the agent dead until morning even though the dependency had recovered at 2:20. The design goal shifts from "stop humans" to **self-recover with guardrails, keep the human informed**.
- **Supervised auto-unlock (A1)**: after a cooldown (60 min, doubling per strike) with the dependency-stability hysteresis confirmed, the NOC clears the lock itself, alerts, and attempts ONE recovery restart. Re-lock within 6h of an auto-unlock = strike 2 = **sticky** (manual-only, "needs you (2nd strike)" on the card) — two failed recoveries mean restarts aren't healing, which is exactly what the lock exists to stop. Manual Clear resets strikes entirely. Legacy locks in the state file (pre-strike bookkeeping) surface as needing the operator instead of crashing the cycle — found live on first run.
- **Telegram control plane (A2)**: reply to alerts with `/unlock <agent>` (substring match), `/status` (per-agent dot-state + today's spend), `/mute <min> [agents]`, `/unmute`. OPT-IN via a dedicated `TELEGRAM_COMMAND_BOT_TOKEN`: getUpdates long-poll 409-conflicts with any other consumer of the shared alert bot (quant; and our own dev instance vs the deployed container fought each other immediately). Unset = off.
- **Maintenance window (A3)**: "Pause monitoring" in the header (15/30/60 min) or `/mute` suppresses restarts, alert_only pages, blocked-by incidents and uptime recording during planned work — probes keep running, cards show "monitoring paused", and "no data" never scores as downtime. One-click resume from the amber banner.
- **Per-agent incidents (A4)**: each monitored card carries a "recent incidents" expansion with that agent's last five events — deciding "clear the lock or investigate" no longer requires scrolling the shared log.
- **Tests**: suite grew to 31 (15 units + 16 render/interaction), covering escalation math, window lifecycle, command handlers (offline via monkeypatched noc calls), mute/resume through the real UI, both card lock states, and the incidents expander. Found en route: `ui.expander` doesn't exist in NiceGUI (`ui.expansion`) and its AttributeError poisoned every test after the first page build.
